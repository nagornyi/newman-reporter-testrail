  ...
  constructor(emitter, reporterOptions, options) {
    ...
    emitter.on('beforeDone', (err, args) => {
      if (results.length > 0) {
        ...
        const tr_base_url = `https://${domain}/index.php?`;
        const tr_headers = {
        ...
        var url_nofilter = `${tr_base_url}/api/v2/get_cases/${projectId}`;
        var url_full = (suiteId)?`${url_nofilter}&suite_id=${suiteId}`:url_nofilter;
        var tr_cases = [];
        var next_page_lnk = null;
        do {
          var res = request('GET', url_full, { headers: tr_headers });
          if (res.statusCode >= 300) console.error(res.getBody());
          var page = JSON.parse(res.getBody());
          tr_cases = tr_cases.concat(page.cases);
          next_page_lnk = page['_links'].next;
          url_full = `${tr_base_url}${next_page_lnk}`;
